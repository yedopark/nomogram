...
# 계산 재시작 시 상태를 유지하고 새로운 계산을 실행하는 함수
def clear_calculation_state():
    st.session_state.calculation_done = False
    st.session_state.previous_inputs = []

# 계산 기록을 저장하는 함수 (최대 5개까지 저장)
def save_calculation_results(results_list, pressure_input, volume_input, output_file_path, graph_buffer):
    if len(results_list) >= 5:
        results_list.pop(0)  # 5개가 넘으면 가장 오래된 기록을 삭제
    results_list.append({
...
# 계산 수행 후 기록을 저장하고, 상태를 업데이트하는 함수
def perform_calculation():
    # 엑셀 파일 읽기 및 계산 로직 (생략된 부분에 로직 삽입)
...
if not st.session_state.calculation_done:
    ...
    # 계산 완료 시 "계산 완료" 버튼과 "계산 재시작" 버튼을 나란히 표시
    ...
    with col2:
        if st.button("계산 재시작"):
            clear_calculation_state()  # 상태 초기화 후 새로 계산

# 이전 기록 다운로드 제공
if st.session_state.previous_results:
    st.write("### 이전 계산 기록:")
    for idx, result in enumerate(st.session_state.previous_results, start=1):
        st.write(f"#### 계산 기록 {idx}: 압력: {result['pressure']} MPa, 부피: {result['volume']} L")
        st.download_button(
            label=f"계산 기록 {idx} 다운로드",
            data=result['output_file_path'],  # 실제 데이터로 교체 필요
            file_name=f"result_{idx}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        )
        st.download_button(
            label=f"계산 기록 {idx} 그래프 다운로드",
            data=result['graph'],  # 실제 그래프 이미지로 교체 필요
            file_name=f"graph_{idx}.png",
            mime="image/png"
        )
